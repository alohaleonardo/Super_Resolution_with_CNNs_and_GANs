...
class Net(nn.Module):
    def __init__(self,params):
        ...
        self.relu = nn.ReLU()
        self.lowlevel = nn.Conv2d(in_channels=3, out_channels=128, kernel_size=11, stride=1, padding=5)
...
def ssim(outputs, labels) :
    ...
    #print ("ssim here")
#     num = outputs.shape[0]
#     outputs = outputs.reshape(num, -1)
#     labels = labels.reshape(num, -1)
    
#     miu_x = np.mean(outputs)
#     miu_y = np.mean(labels)
#     sig_x = np.var(outputs)
#     sig_y = np.var(labels)
    
#     c1 = (0.01*255) * (0.01*255)
#     c2 = (0.03*255) * (0.03*255)
    
#     cov_xy = np.cov(outputs, labels)
    
#     out = (2 * miu_x * miu_y + c1) * (2 * cov_xy + c2) / (miu_x ** 2 + miu_y ** 2) / (sig_x + sig_y + c2) 
#     print('ssim')
#     print(out)
#     return out
