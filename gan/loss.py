...
class GeneratorLoss_adv(nn.Module):
    def __init__(self):
        super(GeneratorLoss_adv, self).__init__()
...
class GeneratorLoss_ssim(nn.Module):
    ...
    def forward(self, out_labels, out_images, target_images):
        ...
        #ssim Loss
        #print("cal ssim loss !!!")
        ssim_loss = pytorch_ssim.ssim(Variable(target_images), Variable(out_images))
...
class GeneratorLoss_notv(nn.Module):
    def __init__(self):
        super(GeneratorLoss_notv, self).__init__()
        vgg = vgg16(pretrained=True)
        loss_network = nn.Sequential(*list(vgg.features)[:31]).eval()
        for param in loss_network.parameters():
            param.requires_grad = False
